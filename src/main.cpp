...
const string strMessageMagic = "Fivebalance Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy =  50000000  * COIN; //premine for swap fbn token
    }  
	else if(nHeight == 2){
        nSubsidy =  50000000  * COIN; //premine for swap fbn token
    }
...
const int YEARLY_BLOCKCOUNT = 518400;

if(nHeight < YEARLY_BLOCKCOUNT * 1)
    {
        nSubsidy = 400 * COIN;
    }
...
{
    int64_t ret = blockValue * 0.80; //80% Masternode Reward from POS
    return ret;
}
