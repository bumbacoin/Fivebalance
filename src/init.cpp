...
void Shutdown()
{
    ...
    RenameThread("fivebalance-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: fivebalance.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: fivebalanced.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizefivebalanceamount=<n> " + _("Keep N Fivebalance anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Fivebalance is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Fivebalance is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Fivebalance version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Fivebalance server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Fivebalance") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Fivebalance to complete") << "\n";
    ...
    nAnonymizeFivebalanceAmount = GetArg("-anonymizefivebalanceamount", 0);
    if(nAnonymizeFivebalanceAmount > 999999) nAnonymizeFivebalanceAmount = 999999;
    if(nAnonymizeFivebalanceAmount < 2) nAnonymizeFivebalanceAmount = 2;
    ...
    LogPrintf("Anonymize Fivebalance Amount %d\n", nAnonymizeFivebalanceAmount);
