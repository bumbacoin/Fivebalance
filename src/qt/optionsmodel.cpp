...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFivebalanceAmount"))
        settings.setValue("nAnonymizeFivebalanceAmount", 1000);
    nAnonymizeFivebalanceAmount = settings.value("nAnonymizeFivebalanceAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFivebalanceAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeFivebalanceAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFivebalanceAmount:
            return QVariant(nAnonymizeFivebalanceAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeFivebalanceAmount:
            nAnonymizeFivebalanceAmount = value.toInt();
            settings.setValue("nAnonymizeFivebalanceAmount", nAnonymizeFivebalanceAmount);
            emit AnonymizeFivebalanceAmountChanged(nAnonymizeFivebalanceAmount);
            break;
        default:
            break;
        }
