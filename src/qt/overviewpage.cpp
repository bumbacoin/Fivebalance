...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFivebalanceAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeFivebalanceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFivebalanceAmount = strAnonymizeFivebalanceAmount.remove(strAnonymizeFivebalanceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFivebalanceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFivebalanceAmount*COIN) nMaxToAnonymize = nAnonymizeFivebalanceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFivebalanceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFivebalanceAmount));
        strAnonymizeFivebalanceAmount = strAnonymizeFivebalanceAmount.remove(strAnonymizeFivebalanceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFivebalanceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeFivebalanceAmount == 0){
