...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("fivebalance"))
        return false;
...
{
    // Convert fivebalance:// to fivebalance:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("fivebalance://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "fivebalance:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fivebalance.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Fivebalance.lnk
...
{
    return GetAutostartDir() / "fivebalance.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a fivebalance.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Fivebalance\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the fivebalance app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add fivebalance app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fivebalance-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  fivebalance-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Fivebalance-Qt"));
