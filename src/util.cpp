...
int nAnonymizeFivebalanceAmount = 1000;
...
#else
    const char* pszModule = "fivebalance";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Fivebalance
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Fivebalance
    // Mac: ~/Library/Application Support/Fivebalance
    // Unix: ~/.fivebalance
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fivebalance";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "fivebalance.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "fivebalanced.pid"));
